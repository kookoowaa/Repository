...
rd = read.csv('d:/cns_data/(180208)data_set_for_regression_c4.csv', stringsAsFactors = F)[,-1]
info = c(1,2,4)
response_variable = 3
key_variable = 5:29
promo1 = 30:32
...
Prior1=list(Deltabar=Delta,
            A=0.01*diag(1+length(brand) + length(clust)),
            nu.e=3,
            ssq=rep(0.06,length(rd_UPC)),
            nu=length(explanatory_variable)+1,
            V=nu*diag(length(explanatory_variable)+1)) 
  ...
  for(j in 1:length(model_outputs[[i]])){
    ...
    if (i == 1) {
      write.csv(get(paste('MAPE_pooled_', model_outputs[[i]][j], sep='')), 
                          paste('d:/cns_data/MAPE/MAPE4/', paste('MAPE_pooled_', model_outputs[[i]][j], sep=''), '.csv', sep=''))
    ...
    if (i == 2) {
      write.csv(get(paste('MAPE_UPC_', model_outputs[[i]][j], sep='')), 
                    paste('d:/cns_data/MAPE/MAPE4/', paste('MAPE_UPC_', model_outputs[[i]][j], sep=''), '.csv', sep=''))
    ...
    if (i == 3) {
      write.csv(get(paste('MAPE_clust_', model_outputs[[i]][j], sep='')), 
                paste('d:/cns_data/MAPE/MAPE4/', paste('MAPE_clust_', model_outputs[[i]][j], sep=''), '.csv', sep=''))
...
write.csv(MAPE, 'd:/CNS_data/MAPE/MAPE4/+MAPE_models.csv')
...
for ( i in 1:length(model_outputs)){
  for(j in 1:length(model_outputs[[i]])){
    ...
    if (i == 1) write.csv(get(paste('MAPE_pooled_', model_outputs[[i]][j],'_CNS', sep='')), 
                          paste('d:/cns_data/MAPE/MAPE4_CNS/', paste('MAPE_pooled_', model_outputs[[i]][j], '_CNS', sep=''), '.csv', sep=''))
    if (i == 2) write.csv(get(paste('MAPE_UPC_', model_outputs[[i]][j],'_CNS', sep='')), 
                          paste('d:/cns_data/MAPE/MAPE4_CNS/', paste('MAPE_UPC_', model_outputs[[i]][j],'_CNS', sep=''), '.csv', sep=''))
    if (i == 3) write.csv(get(paste('MAPE_clust_', model_outputs[[i]][j], '_CNS',sep='')), 
                          paste('d:/cns_data/MAPE/MAPE4_CNS/', paste('MAPE_clust_', model_outputs[[i]][j],'_CNS', sep=''), '.csv', sep=''))

    
  }
}

write.csv(MAPE_CNS, 'd:/CNS_data/MAPE/MAPE4_CNS/+MAPE_models_CNS.csv')
...
```{r}
save.image("D:/CNS_DATA/180209_env_clust13_promo2.RData")
```
