...
for(i in 1:nrow(dat))
{
  ...
  temp_url = paste0('https://jcr.irm(list= ls()) 
                    ...
                    display = 10 ## 검색 결과 출력 건수 지정 : 10(defualt), 100(maximum) 
                    start = 1  ## 검색 시작 위치 : 1(defualt), 1000(maximum) 
                    query = toupper(paste0('%',unlist(iconv(query, to = 'UTF-8', toRaw = T)),collapse = ''))
                    ## 검색을 원한는 문자열, encoding to UTF-8
                    ######  xml format #####
                    ...
                    # HTTP 헤더에 클라이언트 아이디/시크릿 값 포함해서 호출
