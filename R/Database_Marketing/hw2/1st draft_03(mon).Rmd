...
monetary = as.numeric(as.character(mailorder$monetary)) #총 지출액
...
valid_group1[6] = valid1
test1_result = arrange(valid_group1, desc(V6))
...
x2=model.matrix(purchase~.,data = valid_group1[,1:5])[,-1] # 테스트셋 적용을 위한 테스트셋의 x 매트릭스 생성
...
valid_group1[7] <- as.numeric(lasso.pred) # 예측 값 테스트셋에 추가
test2_result = arrange(valid_group1, desc(V7))
...
spending_month = monetary / order_01$duration #월 주문액
spending_transaction = monetary / order_01$transaction #1회 구매액
...
test2 = lm(purchase~gender+recency+transaction+transaction_month, data = test_group2)
valid2 = predict(test2, newdata = valid_group2, type = "response")
valid_group2[9] = valid2
test2_result = arrange(valid_group2, desc(V9))
...
x2=model.matrix(purchase~.,data = valid_group2[,1:8])[,-1] # 테스트셋 적용을 위한 테스트셋의 x 매트릭스 생성
...
bestlam=cv.out$lambda.min # 검정오차 가장 낮은 값(최적 람다) 출력 
lasso.pred = predict(lasso.mod, s=bestlam, newx = x2, type='response') # 예측 셋 
valid_group2[9] <- as.numeric(lasso.pred) # 예측 값 테스트셋에 추가
test2_result = arrange(valid_group2, desc(V9))
...
order_02$spending_month = monetary / sqrt(order_01$duration) #월 주문액
...
valid_group3[9] = valid3
test3_result = arrange(valid_group3, desc(V9))
...
x2=model.matrix(purchase~.,data = valid_group3[,1:8])[,-1] # 테스트셋 적용을 위한 테스트셋의 x 매트릭스 생성
...
bestlam=cv.out$lambda.min # 검정오차 가장 낮은 값(최적 람다) 출력 
lasso.pred = predict(lasso.mod, s=bestlam, newx = x2, type='response') # 예측 셋 
valid_group3[9] <- as.numeric(lasso.pred) # 예측 값 테스트셋에 추가
test3_result = arrange(valid_group3, desc(V9))
