...
transaction_month = mailorder$frequency / mailorder$duration #월 주문 횟수
spending_month = mailorder$monetary / mailorder$duration #월 주문액
spending_transaction = mailorder$monetary / mailorder$frequency #1회 구매액
...
```{R}
## 4번 - 16%
simple_lm = lm(purchase~recency+frequency+monetary, data= test_group1)
#summary(simple_lm)
...
tree_test = valid_group1[valid_group1$recency<16 & valid_group1$frequency>2,]
tree_result = rep(0,2000)
tree_result[valid_group1$recency<16 & valid_group1$frequency>2] = 1
...
```{r}
# 2차실험 (18.4%)
test2 = lm(purchase~ gender + recency + frequency + spending_month + transaction_month, data = test_group1)
...
```{r}
# Frequency 조정, 원본 데이터 수정
Neworder = mailorder
adj_freq = sqrt(Neworder$frequency)
Neworder = cbind(Neworder,adj_freq)
summary(Neworder)
...
```{r}
# 3차 실험(18.2%)
test3 = step(lm(purchase~.-id, data = test_group2))
...
```{r}
# 4차 실험(19.0%)
test4 = lm(purchase~gender + recency + transaction_month + spending_month + 
                  adj_freq, data = test_group2)
