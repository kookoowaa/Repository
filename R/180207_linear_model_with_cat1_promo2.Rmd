...
#load("D:/CNS_DATA/180208_env_cat1_promo3.RData")
...
rd = read.csv('d:/cns_data/(180207)data_set_for_regression.csv', stringsAsFactors = F)[,-1]
info = c(1,2,4)
response_variable = 3
key_variable = 5:29
promo1 = 30:32
...
explanatory_variable2 = c(explanatory_variable, brand, cat1)
  # pooled 
...
model_outputs = list(pooled = c('ols', 'ridge'), UPC = c('ols', 'ridge', 'bayes_pooled', 'bayes_cat', 'bayes_hierachical'), cat = c('ols', 'ridge', 'bayes'))
...
for ( i in 1:length(model_outputs)){
  for(j in 1:length(model_outputs[[i]])){
    ...
    if (i == 3) assign(paste('MAPE_cat_', model_outputs[[i]][j], sep=''), MAPE)
  }
}
...
for ( i in 1:length(model_outputs)){
  for(j in 1:length(model_outputs[[i]])){
    ...
    if (i == 3) MAPE[temp,1] = paste('cat_', model_outputs[[i]][j], sep='')
...
# Bayesian Hierarchical Regression by UPC_cat


UPC_bayes_cat_coef = UPC_bayes_cat_sd = UPC_bayes_pooled_coef
UPC_bayes_cat_coef = UPC_bayes_cat_coef[1,]
UPC_bayes_cat_sd = UPC_bayes_cat_sd[1,]
...
for (i in 1:ns) {
  ...
  {
    if(i==1){
      temp = rd_train[apply(rd_train[,cat1], 1, sum)==0, c(1, response_variable, explanatory_variable)]
      }
    else{
      temp = rd_train[rd_train[,i+79]==1, c(1, response_variable, explanatory_variable)]
      }
  }
  ...
  UPC_bayes_cat_temp = rhierLinearModel(Data=Data1, Mcmc=Mcmc1)    # 실제 regression
  
  
  
  temp_coef = data.frame(matrix(ncol = length(explanatory_variable)+2, nrow = length(temp_UPC)))
  colnames(temp_coef) = colnames(UPC_bayes_cat_coef)
  ...
  for(j in 1:(ncol(temp_coef)-1)){
    temp = UPC_bayes_cat_temp$betadraw[,j,5001:R]
    temp_coef[,j+1] = rowMeans(temp)
    temp_sd[,j+1] = apply(temp,1,sd)
  }

  
  UPC_bayes_cat_coef =  rbind(UPC_bayes_cat_coef, temp_coef)
  UPC_bayes_cat_sd =  rbind(UPC_bayes_cat_sd, temp_sd)
  
  if (i==1){ UPC_bayes_cat_coef= UPC_bayes_cat_coef[-1,] ; UPC_bayes_cat_sd = UPC_bayes_cat_sd[-1,]}
  ...
  for(j in 1:length(temp_UPC)){
    ...
    temp = UPC_bayes_cat_coef %>% filter(UPC==temp_UPC[j]) ; temp = temp[,-1]
    ...
    temp1 = temp1 %>% mutate(pe = abs(unit-yhat)/unit)
    temp1 = temp1 %>% mutate(week = (as.numeric(date) %/% 7-2422), month = ifelse(week %in% 1:4, 1, 2))
    MAPE_UPC_bayes_cat[MAPE_UPC_bayes_cat$UPC == temp_UPC[j],]$MAPE_wk = 
      temp1 %>% group_by(week) %>% summarize(unit = sum(unit), yhat = sum(yhat), wk_pe = abs(unit-yhat)/unit) %>% summarize(mean(wk_pe)) %>% unlist
    MAPE_UPC_bayes_cat[MAPE_UPC_bayes_cat$UPC == temp_UPC[j],]$MAPE_mo = 
      temp1 %>% group_by(month) %>% summarize(unit = sum(unit), yhat = sum(yhat), mo_pe = abs(unit-yhat)/unit) %>% summarize(mean(mo_pe)) %>% unlist
...
MAPE_UPC_bayes_cat$MAPE_wk = as.numeric(MAPE_UPC_bayes_cat$MAPE_wk); MAPE_UPC_bayes_cat$MAPE_mo = as.numeric(MAPE_UPC_bayes_cat$MAPE_mo)
MAPE_UPC_bayes_cat[2,2:3] = apply(MAPE_UPC_bayes_cat[-c(1:2),2:3],2,mean) ; MAPE_UPC_bayes_cat[1,2:3] = 1 - MAPE_UPC_bayes_cat[2,2:3] 
MAPE[MAPE$models=='UPC_bayes_cat',2:3] = MAPE_UPC_bayes_cat[2,2:3] 
  


rm(UPC_bayes_cat_temp)
...
lm_cat_coef = data.frame(matrix(data = NA, nrow = length(cat1)+1, ncol = length(explanatory_variable3)+2))
colnames(lm_cat_coef) = c('cat', 'intercept', colnames(rd[,explanatory_variable3]))
lm_cat_coef[,1] = c( 'cat1_misc', colnames(rd_train [,cat1]))

for(i in 1:(length(cat1)+1)){
  
  # cat 별 linear regression
  {
    if(i==1){
      temp = lm(unit~., data = rd_train[apply(rd_train[,cat1], 1, sum)==0,c(response_variable, explanatory_variable3)])
      }
    else{
      temp =lm(unit~., data = rd_train[rd_train[,i+79]==1,c(response_variable, explanatory_variable3)])
      }
  }
  
  
  # cat 별 coefficient 확인
  lm_cat_coef[lm_cat_coef$cat == c('cat1_misc',colnames(rd[,cat1]))[i],2:ncol(lm_cat_coef)] = temp$coefficients
  
  # cat 별 prediction 확인
  {
    if(i==1){
      temp1 = rd_test[apply(rd_test[,cat1], 1, sum)==0,] %>% select(UPC, date, unit)
      temp1 = temp1 %>% mutate(yhat = predict(temp, newdata = rd_test[apply(rd_test[,cat1], 1, sum)==0,explanatory_variable3]))
    }
    else{
      temp1 = rd_test[rd_test[,i+79]==1,]%>% select(UPC, date, unit)
      temp1 = temp1 %>% mutate(yhat = predict(temp, newdata = rd_test[rd_test[,i+79]==1,explanatory_variable3]))
    }
  }
...
MAPE_cat_ols = MAPE_cat_ols[c(1,nrow(MAPE_cat_ols),2:(nrow(MAPE_cat_ols)-1)),]
MAPE_cat_ols[2,2:3] = apply(MAPE_cat_ols[-c(1:2),2:3],2,mean) ; MAPE_cat_ols[1,2:3] = 1 - MAPE_cat_ols[2,2:3] 
MAPE[MAPE$models=='cat_ols',2:3] = MAPE_cat_ols[2,2:3] 
...
ridge_cat_coef = data.frame(matrix(data = NA, nrow = length(cat1)+1, ncol = length(explanatory_variable3)+2))
colnames(ridge_cat_coef) = c('cat', 'intercept', colnames(rd[,explanatory_variable3]))
ridge_cat_coef[,1] = c('cat1_misc', colnames(rd_train [,cat1]))
...
for(i in 1:(length(cat1)+1)){
  ...
  {
    if(i==1){
      temp = cv.glmnet(x = as.matrix(rd_train[apply(rd_train[,cat1], 1, sum)==0, explanatory_variable3]), 
                          y = rd_train[apply(rd_train[,cat1], 1, sum)==0, response_variable], alpha = 0, lambda = grid)
      }
    else{
      temp = cv.glmnet(x = as.matrix(rd_train[rd_train[,i+79]==1, explanatory_variable3]), 
                          y = rd_train[rd_train[,i+79]==1, response_variable], alpha = 0, lambda = grid)
      }
  }
  ...
  # cat 별 coefficient 확인
  ridge_cat_coef[ridge_cat_coef$cat == c('cat1_misc',colnames(rd[,cat1]))[i],2:ncol(lm_cat_coef)] = coef(temp)
  
  # cat 별 prediction 확인
  {
    if(i==1){
      temp1 = rd_test[apply(rd_test[,cat1], 1, sum)==0,] %>% select(UPC, date, unit)
      temp1 = cbind(temp1, predict(temp, newx = as.matrix(rd_test[apply(rd_test[,cat1], 1, sum)==0,explanatory_variable3]), s = 'lambda.1se'))
    }
    else{
      temp1 = rd_test[rd_test[,i+79]==1,]%>% select(UPC, date, unit)
      temp1 = cbind(temp1, predict(temp, newx = as.matrix(rd_test[rd_test[,i+79]==1,explanatory_variable3]), s = 'lambda.1se'))
    }
  }
  colnames(temp1)[4] = 'yhat'
  temp1$unit = exp(temp1$unit); temp1$yhat = exp(temp1$yhat)
...
MAPE_cat_ridge = MAPE_cat_ridge[c(1,nrow(MAPE_cat_ridge),2:(nrow(MAPE_cat_ridge)-1)),]
MAPE_cat_ridge[2,2:3] = apply(MAPE_cat_ridge[-c(1:2),2:3],2,mean) ; MAPE_cat_ridge[1,2:3] = 1 - MAPE_cat_ridge[2,2:3] 
MAPE[MAPE$models=='cat_ridge',2:3] = MAPE_cat_ridge[2,2:3] 
```
...
#  {
#    if(i==1){
#      temp = xgboost(data = data.matrix(rd_train[apply(rd_train[,cat1], 1, sum)==0, explanatory_variable3]), 
#                 label = rd_train[apply(rd_train[,cat1], 1, sum)==0, response_variable],
#                 max.depth = 3, eta = 0.001, nround = 5000, objective = "reg:linear", verbose = 0)
      
        
#      }
#    else{
#      temp = xgboost(data = data.matrix(rd_train[rd_train[,i+79]==1, explanatory_variable3]), 
#                 label = rd_train[rd_train[,i+79]==1, response_variable],
#                 max.depth = 3, eta = 0.001, nround = 5000, objective = "reg:linear", verbose = 0)
#      }
#  }
...
#    if(i==1){
#      temp1 = rd_test[apply(rd_test[,cat1], 1, sum)==0,] %>% select(UPC, date, unit)
#      temp1 = temp1 %>% mutate(yhat = predict(temp, newdata = data.matrix(rd_test[apply(rd_test[,cat1], 1, sum)==0,explanatory_variable3])))
#    }
#    else{
#      temp1 = rd_test[rd_test[,i+79]==1,]%>% select(UPC, date, unit)
#      temp1 = temp1 %>% mutate(yhat = predict(temp, newdata = data.matrix(rd_test[rd_test[,i+79]==1,explanatory_variable3])))
#    }
#  } 
...
#MAPE_cat_boosting = MAPE_cat_boosting[c(1,nrow(MAPE_cat_boosting),2:(nrow(MAPE_cat_boosting)-1)),]
#MAPE_cat_boosting[2,2:3] = apply(MAPE_cat_boosting[-c(1:2),2:3],2,mean) ; MAPE_cat_boosting[1,2:3] = 1 - MAPE_cat_boosting[2,2:3] 
#MAPE[MAPE$models=='cat_boosting',2:3] = MAPE_cat_boosting[2,2:3] 
```


```{r}
# Bayesian Hierarchical Regression by cat_pooled
...
for (i in 1:ns) {
  {
    if(i==1){
      temp = rd_train[apply(rd_train[,cat1], 1, sum)==0, c(response_variable, explanatory_variable3)]
      }
    else{
      temp = rd_train[rd_train[,i+79]==1, c(response_variable, explanatory_variable3)]
      }
  }
...
#Prior1=list(Deltabar=Delta,A=0.01*diag(1),nu.e=3,ssq=rep(0.06,ns),nu=6,V=6*0.1*diag(3)) 
#out=rhierLinearModel(Data=Data1,Prior=Prior1, Mcmc=Mcmc1)
cat_bayes = rhierLinearModel(Data=Data1, Mcmc=Mcmc1)    # 실제 regression
...
cat_bayes_coef = as.data.frame(matrix(nrow = length(cat1)+1, ncol = dim(cat_bayes$betadraw)[2]+1))
colnames(cat_bayes_coef) = c('cat', 'intercept', colnames(rd[,explanatory_variable3]))
cat_bayes_coef[,1] =  c('cat1_misc', colnames(rd_train [,cat1]))
cat_bayes_sd = cat_bayes_coef




for(i in 1:(ncol(cat_bayes_coef)-1)){
  temp = cat_bayes$betadraw[,i,5001:R]
  cat_bayes_coef[,i+1] = rowMeans(temp)
  cat_bayes_sd[,i+1] = apply(temp,1,sd)
}
...
for(i in 1:ns){
  ...
  temp = cat_bayes_coef[i,-1]
  
  
  # UPC 별 prediction 확인
  {
    if(i==1){
      temp1 = cbind(1, rd_test[apply(rd_test[,cat1], 1, sum)==0, explanatory_variable3])
      temp1 = cbind(rd_test[apply(rd_test[,cat1], 1, sum)==0,1:3], as.matrix(temp1) %*% t(temp))
      }
    else{
      temp1 = cbind(1, rd_test[rd_test[,i+79]==1, explanatory_variable3])
      temp1 = cbind(rd_test[rd_test[,i+79]==1, 1:3], as.matrix(temp1) %*% t(temp))
      }
  }
  ...
  temp2 = merge(temp2[,-4],
                ...
                by = 'UPC', all.x = T)
  temp2$MAPE_mo[!is.na(temp2$mo_pe)] = temp2$mo_pe[!is.na(temp2$mo_pe)]
  MAPE_cat_bayes = temp2[,-4]
  
  
  print(i)
}

MAPE_cat_bayes = MAPE_cat_bayes[c(1,nrow(MAPE_cat_bayes),2:(nrow(MAPE_cat_bayes)-1)),]
MAPE_cat_bayes[2,2:3] = apply(MAPE_cat_bayes[-c(1:2),2:3],2,mean) ; MAPE_cat_bayes[1,2:3] = 1 - MAPE_cat_bayes[2,2:3] 
MAPE[MAPE$models=='cat_bayes',2:3] = MAPE_cat_bayes[2,2:3] 
...
temp = as.data.frame(matrix(data = NA,ncol = 1+1+length(brand)+length(cat1), nrow = length(rd_UPC)))
temp[,1] = rd_UPC ; temp[,2]=1
colnames(temp) = c('UPC', 'iota', colnames(rd[,c(brand,cat1)]))

temp1 = merge(temp[,1:2], rd[,c(1,brand,cat1)]%>%distinct(), by = 'UPC')

for(i in 1:length(rd_UPC)){
temp[i,3:46] = temp1[temp1$UPC %in% temp$UPC[i],3:46]
}

Z = as.matrix(temp[,2:46])
#nz=ncol(Z)
Delta=matrix(0,nrow = 1+length(brand) + length(cat1), ncol = nrow(Z))    #length(rd_UPC), ncol=nz)
Delta=t(Delta) # first row of Delta is means of betas
...
cat_bayes_coef_list = colnames(cat_bayes_coef[,-1])
temp = as.data.frame(t(cat_bayes$betadraw[,promo_id[1],5001:R])); colnames(temp) = c('cat1_misc', colnames(rd_train [,cat1])); temp = melt(temp)
  
  ggplot(data = lm_cat_coef) +
  labs(title = paste('effect of ', cat_bayes_coef_list[promo_id[2]], ' in different models', sep='')) +
    geom_boxplot(data = temp, aes(x = variable, y = value, fill= 'a'))+
    geom_line(data =cat_bayes_coef , aes(x = cat, y = get(cat_bayes_coef_list[promo_id[1]]), color = 'bayesian_cat', group = 1))+
    
    geom_hline(aes(yintercept = lm_coef[1,promo_id[2]+1], color = 'pooled_ols')) +
    
    geom_point(aes(x = cat, y = get(cat_bayes_coef_list[promo_id[2]]), color = 'ols_cat')) +
    #geom_line(aes(x = cat, y = get(cat_bayes_coef_list[promo_id[2]]), color = 'ols_cat'), group = 1)+
  ...
  for(j in 1:length(model_outputs[[i]])){
    ...
    if (i == 1) {
      write.csv(get(paste('MAPE_pooled_', model_outputs[[i]][j], sep='')), 
                          paste('d:/cns_data/MAPE/MAPE1/', paste('MAPE_pooled_', model_outputs[[i]][j], sep=''), '.csv', sep=''))
    ...
    if (i == 2) {
      write.csv(get(paste('MAPE_UPC_', model_outputs[[i]][j], sep='')), 
                    paste('d:/cns_data/MAPE/MAPE1/', paste('MAPE_UPC_', model_outputs[[i]][j], sep=''), '.csv', sep=''))
    ...
    if (i == 3) {
      write.csv(get(paste('MAPE_cat_', model_outputs[[i]][j], sep='')), 
                paste('d:/cns_data/MAPE/MAPE1/', paste('MAPE_cat_', model_outputs[[i]][j], sep=''), '.csv', sep=''))
      
      MAPE_wk = merge(MAPE_wk, get(paste('MAPE_cat_', model_outputs[[i]][j], sep=''))[-(1:2),1:2], by = 'UPC', all.x=T)
      MAPE_mo = merge(MAPE_mo, get(paste('MAPE_cat_', model_outputs[[i]][j], sep=''))[-(1:2),c(1,3)], by = 'UPC', all.x=T)
      colnames(MAPE_wk)[col_num] = colnames(MAPE_mo)[col_num] = paste('cat_', model_outputs[[i]][j], sep='')
...
write.csv(MAPE, 'd:/CNS_data/MAPE/MAPE1/+MAPE_models.csv')
...
ggplot(data = melt(MAPE_mo[,-1]) %>% mutate(grp = rep(c('pooled','pooled', 'by_UPC','by_UPC', 'by_UPC', 'by_UPC', 'by_UPC', 'by_cat', 'by_cat', 'by_cat'), each = length(rd_UPC)))) +
...
for ( i in 1:length(model_outputs)){
  for(j in 1:length(model_outputs[[i]])){
    ...
    if (i == 3) {
      temp = rbind(get(paste('MAPE_cat_', model_outputs[[i]][j], sep=''))[1:2,],
                   get(paste('MAPE_cat_', model_outputs[[i]][j], sep=''))[get(paste('MAPE_cat_', model_outputs[[i]][j], sep=''))$UPC %in% test_UPC,])
      temp[2,2:3] = apply(temp[-c(1:2),2:3],2,mean) ; temp[1,2:3] = 1 - temp[2,2:3] 
      assign(paste('MAPE_cat_', model_outputs[[i]][j], '_CNS',sep=''), temp)
      MAPE_CNS[MAPE_CNS$models==paste('cat_',model_outputs[[i]][j], sep=''),2:3] = temp[2,2:3]
      
      MAPE_wk_CNS = merge(MAPE_wk_CNS, get(paste('MAPE_cat_', model_outputs[[i]][j], sep=''))[-(1:2),1:2], by = 'UPC', all.x=T)
      MAPE_mo_CNS = merge(MAPE_mo_CNS, get(paste('MAPE_cat_', model_outputs[[i]][j], sep=''))[-(1:2),c(1,3)], by = 'UPC', all.x=T)
      colnames(MAPE_wk_CNS)[col_num] = colnames(MAPE_mo_CNS)[col_num] = paste('cat_', model_outputs[[i]][j], sep='')
...
for ( i in 1:length(model_outputs)){
  for(j in 1:length(model_outputs[[i]])){
    ...
    if (i == 1) write.csv(get(paste('MAPE_pooled_', model_outputs[[i]][j],'_CNS', sep='')), 
                          paste('d:/cns_data/MAPE/MAPE1_CNS/', paste('MAPE_pooled_', model_outputs[[i]][j], '_CNS', sep=''), '.csv', sep=''))
    if (i == 2) write.csv(get(paste('MAPE_UPC_', model_outputs[[i]][j],'_CNS', sep='')), 
                          paste('d:/cns_data/MAPE/MAPE1_CNS/', paste('MAPE_UPC_', model_outputs[[i]][j],'_CNS', sep=''), '.csv', sep=''))
    if (i == 3) write.csv(get(paste('MAPE_cat_', model_outputs[[i]][j], '_CNS',sep='')), 
                          paste('d:/cns_data/MAPE/MAPE1_CNS/', paste('MAPE_cat_', model_outputs[[i]][j],'_CNS', sep=''), '.csv', sep=''))

    
  }
}

write.csv(MAPE_CNS, 'd:/CNS_data/MAPE/MAPE1_CNS/+MAPE_models_CNS.csv')
```
