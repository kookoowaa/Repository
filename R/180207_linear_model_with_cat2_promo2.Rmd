...
#load("D:/CNS_DATA/180208_env_cat2_promo3.RData")
...
explanatory_variable2 = c(explanatory_variable, brand, cat2)
  # pooled 
...
for (i in 1:ns) {
  ...
  temp = rd_train[rd_train[,i+cat2[1]-1]==1, c(1, response_variable, explanatory_variable)]
  temp_UPC = rd_UPC[rd_UPC %in% temp$UPC]
  
  
  if (length(temp_UPC)==0){}else{
  ...
  for(j in 1:(ncol(temp_coef)-1)){
    ...
    {
    if(length(temp)==10000){
    temp_coef[,j+1] = mean(temp)
    temp_sd[,j+1] = sd(temp)
    }
...
lm_cat_coef = data.frame(matrix(data = NA, nrow = length(cat2), ncol = length(explanatory_variable3)+2))
colnames(lm_cat_coef) = c('cat', 'intercept', colnames(rd[,explanatory_variable3]))
lm_cat_coef[,1] = c( colnames(rd_train [,cat2]))

for(i in 1:length(cat2)){
  
  # cat 별 linear regression
  temp =lm(unit~., data = rd_train[rd_train[,i+cat2[1]-1]==1,c(response_variable, explanatory_variable3)])
  
  
  # cat 별 coefficient 확인
  lm_cat_coef[lm_cat_coef$cat == c('cat2_misc',colnames(rd[,cat2]))[i],2:ncol(lm_cat_coef)] = temp$coefficients
  
  # cat 별 prediction 확인
 
      temp1 = rd_test[rd_test[,i+cat2[1]-1]==1,]%>% select(UPC, date, unit)
      temp1 = temp1 %>% mutate(yhat = predict(temp, newdata = rd_test[rd_test[,i+cat2[1]-1]==1,explanatory_variable3]))
...
ridge_cat_coef = data.frame(matrix(data = NA, nrow = length(cat2), ncol = length(explanatory_variable3)+2))
colnames(ridge_cat_coef) = c('cat', 'intercept', colnames(rd[,explanatory_variable3]))
ridge_cat_coef[,1] = c( colnames(rd_train [,cat2]))

for(i in 1:length(cat2)){
  
  # cat 별 ridge regression
  
      temp = cv.glmnet(x = as.matrix(rd_train[rd_train[,i+cat2[1]-1]==1, explanatory_variable3]), 
                          y = rd_train[rd_train[,i+cat2[1]-1]==1, response_variable], alpha = 0, lambda = grid)
  
               

  
  # cat 별 coefficient 확인
  ridge_cat_coef[ridge_cat_coef$cat == c('cat2_misc',colnames(rd[,cat2]))[i],2:ncol(lm_cat_coef)] = coef(temp)
  
  # cat 별 prediction 확인
      temp1 = rd_test[rd_test[,i+cat2[1]-1]==1,]%>% select(UPC, date, unit)
      temp1 = cbind(temp1, predict(temp, newx = as.matrix(rd_test[rd_test[,i+cat2[1]-1]==1,explanatory_variable3]), s = 'lambda.1se'))
...
#for(i in 1:(length(cat2)+1)){
  
  # cat 별 boosting
#  {
#    if(i==1){
#      temp = xgboost(data = data.matrix(rd_train[apply(rd_train[,cat2], 1, sum)==0, explanatory_variable3]), 
#                 label = rd_train[apply(rd_train[,cat2], 1, sum)==0, response_variable],
#                 max.depth = 3, eta = 0.001, nround = 5000, objective = "reg:linear", verbose = 0)
      
        
#      }
#    else{
#      temp = xgboost(data = data.matrix(rd_train[rd_train[,i+cat2[1]-2]==1, explanatory_variable3]), 
#                 label = rd_train[rd_train[,i+cat2[1]-2]==1, response_variable],
#                 max.depth = 3, eta = 0.001, nround = 5000, objective = "reg:linear", verbose = 0)
#      }
#  }
...
#  {
#    if(i==1){
#      temp1 = rd_test[apply(rd_test[,cat2], 1, sum)==0,] %>% select(UPC, date, unit)
#      temp1 = temp1 %>% mutate(yhat = predict(temp, newdata = data.matrix(rd_test[apply(rd_test[,cat2], 1, sum)==0,explanatory_variable3])))
#    }
#    else{
#      temp1 = rd_test[rd_test[,i+cat2[1]-2]==1,]%>% select(UPC, date, unit)
#      temp1 = temp1 %>% mutate(yhat = predict(temp, newdata = data.matrix(rd_test[rd_test[,i+cat2[1]-2]==1,explanatory_variable3])))
#    }
#  } 
...
for (i in 1:ns) {
  ...
  temp = rd_train[rd_train[,i+cat2[1]-1]==1, c(response_variable, explanatory_variable3)]
...
cat_bayes_coef = as.data.frame(matrix(nrow = length(cat2), ncol = dim(cat_bayes$betadraw)[2]+1))
colnames(cat_bayes_coef) = c('cat', 'intercept', colnames(rd[,explanatory_variable3]))
cat_bayes_coef[,1] =  c(colnames(rd_train [,cat2]))
...
for(i in 1:ns){
  ...
  temp1 = cbind(1, rd_test[rd_test[,i+cat2[1]-1]==1, explanatory_variable3])
  temp1 = cbind(rd_test[rd_test[,i+cat2[1]-1]==1, 1:3], as.matrix(temp1) %*% t(temp))
...
temp = as.data.frame(matrix(data = NA,ncol = 1+1+length(brand)+length(cat2), nrow = length(rd_UPC)))
temp[,1] = rd_UPC ; temp[,2]=1
colnames(temp) = c('UPC', 'iota', colnames(rd[,c(brand,cat2)]))

temp1 = merge(temp[,1:2], rd[,c(1,brand,cat2)]%>%distinct(), by = 'UPC')
...
#nz=ncol(Z)
Delta=matrix(0,nrow = 1+length(brand) + length(cat2), ncol = nrow(Z))    #length(rd_UPC), ncol=nz)
Delta=t(Delta) # first row of Delta is means of betas
...
temp = as.data.frame(t(cat_bayes$betadraw[,promo_id[1],5001:R])); colnames(temp) = c('cat2_misc', colnames(rd_train [,cat2])); temp = melt(temp)
  ...
  for(j in 1:length(model_outputs[[i]])){
    ...
    if (i == 1) {
      write.csv(get(paste('MAPE_pooled_', model_outputs[[i]][j], sep='')), 
                          paste('d:/cns_data/MAPE/MAPE2/', paste('MAPE_pooled_', model_outputs[[i]][j], sep=''), '.csv', sep=''))
    ...
    if (i == 2) {
      write.csv(get(paste('MAPE_UPC_', model_outputs[[i]][j], sep='')), 
                    paste('d:/cns_data/MAPE/MAPE2/', paste('MAPE_UPC_', model_outputs[[i]][j], sep=''), '.csv', sep=''))
    ...
    if (i == 3) {
      write.csv(get(paste('MAPE_cat_', model_outputs[[i]][j], sep='')), 
                paste('d:/cns_data/MAPE/MAPE2/', paste('MAPE_cat_', model_outputs[[i]][j], sep=''), '.csv', sep=''))
...
write.csv(MAPE, 'd:/CNS_data/MAPE/MAPE2/+MAPE_models.csv')
...
for ( i in 1:length(model_outputs)){
  for(j in 1:length(model_outputs[[i]])){
    ...
    if (i == 1) write.csv(get(paste('MAPE_pooled_', model_outputs[[i]][j],'_CNS', sep='')), 
                          paste('d:/cns_data/MAPE/MAPE2_CNS/', paste('MAPE_pooled_', model_outputs[[i]][j], '_CNS', sep=''), '.csv', sep=''))
    if (i == 2) write.csv(get(paste('MAPE_UPC_', model_outputs[[i]][j],'_CNS', sep='')), 
                          paste('d:/cns_data/MAPE/MAPE2_CNS/', paste('MAPE_UPC_', model_outputs[[i]][j],'_CNS', sep=''), '.csv', sep=''))
    if (i == 3) write.csv(get(paste('MAPE_cat_', model_outputs[[i]][j], '_CNS',sep='')), 
                          paste('d:/cns_data/MAPE/MAPE2_CNS/', paste('MAPE_cat_', model_outputs[[i]][j],'_CNS', sep=''), '.csv', sep=''))

    
  }
}

write.csv(MAPE_CNS, 'd:/CNS_data/MAPE/MAPE2_CNS/+MAPE_models_CNS.csv')
```
