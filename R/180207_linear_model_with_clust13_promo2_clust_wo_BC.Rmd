...
explanatory_variable2 = c(explanatory_variable, clust)
  # pooled 

explanatory_variable3 = c(explanatory_variable)
  # clust
...
for(i in 1:(ncol(UPC_bayes_pooled_coef)-1)){
  temp = UPC_bayes_pooled$betadraw[,i,dim(UPC_bayes_pooled$betadraw)[3]/2+1:dim(UPC_bayes_pooled$betadraw)[3]]
...
bayesian_comparison = function(x){
  ...
  temp = as.data.frame(t(UPC_bayes_pooled$betadraw[,x,dim(UPC_bayes_pooled$betadraw)[3]/2+1:dim(UPC_bayes_pooled$betadraw)[3]])); colnames(temp) = rd_UPC; temp = melt(temp)
...
for (i in 1:ns) {
  ...
  for(j in 1:(ncol(temp_coef)-1)){
    temp = UPC_bayes_clust_temp$betadraw[,j,(dim(UPC_bayes_clust_temp$betadraw)[3]/2+1):(dim(UPC_bayes_clust_temp$betadraw)[3])]
...
for(i in 1:(ncol(clust_bayes_coef)-1)){
  temp = clust_bayes$betadraw[,i,(dim(clust_bayes$betadraw)[3]/2+1):dim(clust_bayes$betadraw)[3]]
...
#nz=ncol(Z)
Delta=matrix(0,nrow = 1+length(brand) + length(clust), ncol = length(explanatory_variable)+1)    #length(rd_UPC), ncol=nz)
...
#Prior1=list(Deltabar=Delta, A=0.01*diag(1+length(brand) + length(clust)), nu.e=3, ssq=rep(0.06,length(rd_UPC)),nu=length(explanatory_variable)+1, V=nu*diag(length(explanatory_variable)+1)) 
...
#cat("Summary of Delta draws",fill=TRUE)
#summary(UPC_bayes_hierachical$Deltadraw)
#cat("Summary of Vbeta draws",fill=TRUE)
#summary(UPC_bayes_hierachical$Vbetadraw)
```
...
for(i in 1:(ncol(UPC_bayes_hierachical_coef)-1)){
  temp = UPC_bayes_hierachical$betadraw[,i,(dim(UPC_bayes_hierachical$betadraw)[3]/2+1):dim(UPC_bayes_hierachical$betadraw)[3]]
  ...
  for(j in 1:length(model_outputs[[i]])){
    ...
    if (i == 1) {
      write.csv(get(paste('MAPE_pooled_', model_outputs[[i]][j], sep='')), 
                          paste('d:/cns_data/MAPE/MAPE5/', paste('MAPE_pooled_', model_outputs[[i]][j], sep=''), '.csv', sep=''))
    ...
    if (i == 2) {
      write.csv(get(paste('MAPE_UPC_', model_outputs[[i]][j], sep='')), 
                    paste('d:/cns_data/MAPE/MAPE5/', paste('MAPE_UPC_', model_outputs[[i]][j], sep=''), '.csv', sep=''))
    ...
    if (i == 3) {
      write.csv(get(paste('MAPE_clust_', model_outputs[[i]][j], sep='')), 
                paste('d:/cns_data/MAPE/MAPE5/', paste('MAPE_clust_', model_outputs[[i]][j], sep=''), '.csv', sep=''))
...
write.csv(MAPE, 'd:/CNS_data/MAPE/MAPE5/+MAPE_models.csv')
...
for ( i in 1:length(model_outputs)){
  for(j in 1:length(model_outputs[[i]])){
    ...
    if (i == 1) write.csv(get(paste('MAPE_pooled_', model_outputs[[i]][j],'_CNS', sep='')), 
                          paste('d:/cns_data/MAPE/MAPE5_CNS/', paste('MAPE_pooled_', model_outputs[[i]][j], '_CNS', sep=''), '.csv', sep=''))
    if (i == 2) write.csv(get(paste('MAPE_UPC_', model_outputs[[i]][j],'_CNS', sep='')), 
                          paste('d:/cns_data/MAPE/MAPE5_CNS/', paste('MAPE_UPC_', model_outputs[[i]][j],'_CNS', sep=''), '.csv', sep=''))
    if (i == 3) write.csv(get(paste('MAPE_clust_', model_outputs[[i]][j], '_CNS',sep='')), 
                          paste('d:/cns_data/MAPE/MAPE5_CNS/', paste('MAPE_clust_', model_outputs[[i]][j],'_CNS', sep=''), '.csv', sep=''))

    
  }
}

write.csv(MAPE_CNS, 'd:/CNS_data/MAPE/MAPE5_CNS/+MAPE_models_CNS.csv')
...
```{r}
save.image("D:/CNS_DATA/180210_env_clust13_only.RData")
```
