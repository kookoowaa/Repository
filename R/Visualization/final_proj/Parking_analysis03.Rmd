...
info_by_ADDR$color_variable = rep(2,length(info_by_ADDR$ADDR))
info_by_ADDR$color_variable[info_by_ADDR$ADDR %in% (info_by_ADDR[,c(1,2)] %>% arrange(desc(CAPA)) %>% .[1:5,1])] = 1
info_by_ADDR$color_variable[info_by_ADDR$ADDR %in% (info_by_ADDR[,c(1,2)] %>% arrange(desc(CAPA)) %>% .[21:25,1])] = 3
...
```{r message=FALSE,  warning=FALSE}
info_by_ADDR$color_variable = rep(2,length(info_by_ADDR$ADDR))
info_by_ADDR$color_variable[info_by_ADDR$ADDR %in% (info_by_ADDR[,c(1,6)] %>% arrange(desc(CAPA_HOUSE)) %>% .[1:5,1])] = 1
info_by_ADDR$color_variable[info_by_ADDR$ADDR %in% (info_by_ADDR[,c(1,6)] %>% arrange(desc(CAPA_HOUSE)) %>% .[21:25,1])] = 3
...
```{r message=FALSE,  warning=FALSE}
info_by_ADDR$color_variable = rep(2,length(info_by_ADDR$ADDR))
info_by_ADDR$color_variable[info_by_ADDR$ADDR %in% (info_by_ADDR[,c(1,7)] %>% arrange(desc(CAPA_POP)) %>% .[1:5,1])] = 1
info_by_ADDR$color_variable[info_by_ADDR$ADDR %in% (info_by_ADDR[,c(1,7)] %>% arrange(desc(CAPA_POP)) %>% .[21:25,1])] = 3
...
for (i in 11:12) info_by_ADDR[,i] = as.integer(info_by_ADDR[,i])
```
  + 비교를 위해 신규 변수 '자가용 당 공영주차장 CAPA'와 '공영주차장비율' 생성
```{r message=FALSE,  warning=FALSE}
#info_by_ADDR = mutate(info_by_ADDR, CAPA_car = CAPA/vehicle, Public_PARK = CAPA/PARK)
info_by_ADDR$CAPA_car = info_by_ADDR$CAPA / info_by_ADDR$vehicle
info_by_ADDR$Public_park = info_by_ADDR$CAPA / info_by_ADDR$PARK
```
...
```{r message=FALSE,  warning=FALSE}
info_by_ADDR$color_variable = rep(2,length(info_by_ADDR$ADDR))
info_by_ADDR$color_variable[info_by_ADDR$ADDR %in% (info_by_ADDR[,c(1,13)] %>% arrange(desc(CAPA_car)) %>% .[1:5,1])] = 1
info_by_ADDR$color_variable[info_by_ADDR$ADDR %in% (info_by_ADDR[,c(1,13)] %>% arrange(desc(CAPA_car)) %>% .[21:25,1])] = 3
...
```
```{r message=FALSE,  warning=FALSE}
info_by_ADDR[,c(1,13)] %>% arrange(desc(CAPA_car)) %>% .[c(1:5, 21:25),]
```
  + 지역구 별 공영주차장 비율 비교
```{r message=FALSE,  warning=FALSE}
...
```
```{r message=FALSE,  warning=FALSE}
info_by_ADDR[,c(1,14)] %>% arrange(desc(Public_PARK)) %>% .[c(1:5, 21:25),]
```
**4. 가결론 (분석)**
